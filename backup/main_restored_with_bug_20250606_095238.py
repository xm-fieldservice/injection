...
class MainWindow(QMainWindow):
    ...
    def inject_command(self):
        ...
        # 记录注入尝试开始
        attempt_id = datetime.datetime.now().isoformat()
        self.log_injection_failure_check("INJECTION_ATTEMPT_START", attempt_id, {
            'command_length': len(self.command_input.toPlainText().strip()),
            'has_target_window': bool(self.target_window),
            'has_target_position': bool(self.target_position),
            'has_log_file': bool(self.log_file)
        })
        
        # 校准检查
        if not self.target_window or not self.target_position:
            error_msg = "请先校准目标窗口"
            self.log_injection_failure_check("CALIBRATION_FAILED", attempt_id, {
                'target_window': self.target_window,
                'target_position': self.target_position,
                'error': error_msg
            })
        ...
        if not self.log_file:
            error_msg = "请先选择日志文件"
            self.log_injection_failure_check("LOG_FILE_MISSING", attempt_id, {
                'log_file': self.log_file,
                'error': error_msg
            })
            QMessageBox.warning(self, "错误", error_msg)
        ...
        if not command:
            error_msg = "请输入命令"
            self.log_injection_failure_check("EMPTY_COMMAND", attempt_id, {
        ...
        try:
            # 记录窗口状态检查
            self.log_injection_failure_check("TARGET_WINDOW_CHECK", attempt_id, {
                'target_window_exists': win32gui.IsWindow(self.target_window) if self.target_window else False,
                'target_window_visible': win32gui.IsWindowVisible(self.target_window) if self.target_window else False,
            ...
            if self.realtime_check.isChecked():
                if not self.ai_service.api_key:
                    self.log_injection_failure_check("API_KEY_MISSING", attempt_id, {
            ...
            # 记录开始执行注入操作
            self.log_injection_failure_check("INJECTION_EXECUTION_START", attempt_id, {
                'command_final_length': len(command),
                'template_applied': self.default_scene is not None
            })
            ...
            try:
                ...
                window_activated = (current_foreground == self.target_window)
                
                self.log_injection_failure_check("WINDOW_ACTIVATION", attempt_id, {
                    'target_window': self.target_window,
                    'current_foreground': current_foreground,
                    'activation_success': window_activated
                })
                
            except Exception as e:
                self.log_injection_failure_check("WINDOW_ACTIVATION_ERROR", attempt_id, {
                    'error': str(e),
                    'target_window': self.target_window
                })
                raise e
            ...
            try:
                ...
                self.log_injection_failure_check("MOUSE_POSITIONING", attempt_id, {
                    'target_position': self.target_position,
                    'screen_point': point
                })
                
            except Exception as e:
                self.log_injection_failure_check("MOUSE_POSITIONING_ERROR", attempt_id, {
                    'error': str(e),
                    'target_position': self.target_position
                })
                raise e
            ...
            try:
                ...
                self.log_injection_failure_check("MOUSE_CLICK", attempt_id, {
                    'click_executed': True
                })
                
            except Exception as e:
                self.log_injection_failure_check("MOUSE_CLICK_ERROR", attempt_id, {
                    'error': str(e)
                })
                raise e
            ...
            try:
                ...
                self.log_injection_failure_check("CLIPBOARD_OPERATION", attempt_id, {
                    'clipboard_set': True,
                    'command_length': len(command),
                    'method': 'pyperclip'
                })
                ...
                self.log_injection_failure_check("CLIPBOARD_ERROR", attempt_id, {
                    'error': str(e),
                    'method': 'pyperclip'
                })
                raise e
            ...
            try:
                ...
                self.log_injection_failure_check("PASTE_OPERATION", attempt_id, {
                    'paste_executed': True
                })
                
            except Exception as e:
                self.log_injection_failure_check("PASTE_ERROR", attempt_id, {
                    'error': str(e)
                })
                raise e
            ...
            try:
                ...
                self.log_injection_failure_check("ENTER_KEY", attempt_id, {
                    'enter_executed': True
                })
                
            except Exception as e:
                self.log_injection_failure_check("ENTER_KEY_ERROR", attempt_id, {
                    'error': str(e)
                })
                raise e
            ...
            # 记录注入失败
            self.log_injection_failure_check("INJECTION_FAILED", attempt_id, {
                'success': False,
                'error': str(e),
